...
void InterfaceToLocalization::updateObservations() {
  ...
  vector<Observation> pinkOverBlue = findMarkersFromBlobs(pinkBlobs, blueBlobs, "p/b");
  vector<Observation> blueOverPink = findMarkersFromBlobs(blueBlobs, pinkBlobs, "b/p");
  vector<Observation> pinkOverGreen = findMarkersFromBlobs(pinkBlobs, greenBlobs, "p/g");
  vector<Observation> greenOverPink = findMarkersFromBlobs(greenBlobs, pinkBlobs, "g/p");
  vector<Observation> pinkOverYellow = findMarkersFromBlobs(pinkBlobs, yellowBlobs, "p/y");
  vector<Observation> yellowOverPink = findMarkersFromBlobs(yellowBlobs, pinkBlobs, "y/p");
  vector<Observation> pinkOverOrange = findMarkersFromBlobs(pinkBlobs, orangeBlobs, "p/o");
  vector<Observation> orangeOverPink = findMarkersFromBlobs(orangeBlobs, pinkBlobs, "o/p");
  ...
  for (int i = 0; i < pinkBlobs.size(); i++) {
    obs.push_back(Observation("pink", 
			      map, 
			      getAngle((pinkBlobs[i].left + pinkBlobs[i].right) / 2),
			      observationVariance));
  }
...
int InterfaceToLocalization::getBlobColor(player_blobfinder_blob blob) {
  ...
  //cout << "getting blob color - values for r:" << r << ", g:" << g << ", b:" << b << endl;
...
vector<Observation> InterfaceToLocalization::findMarkersFromBlobs(vector<player_blobfinder_blob>& topBlobs,
  ...
  for (int i = 0; i < topBlobs.size(); i++) {
    ...
    for (vector<player_blobfinder_blob>::iterator j = bottomBlobs.begin(); j!= bottomBlobs.end(); j++) {
